...
def main():
    ...
    chemical_mass = float(input("Enter a chemical sample in grams:  "))
    assert isinstance(chemical_mass, float)
    ...
    # call known molecules dictionary
    known_molecules_dict = make_known_molecules_dict()
    compoundName = get_formula_name(formula, known_molecules_dict)
    print(f"Compound name: {compoundName}")


def make_periodic_table():
    # create periodic table dictionary
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for element in range(len(symbol_quantity_list)):
        symbol = symbol_quantity_list[element][SYMBOL_INDEX]
        quantity = symbol_quantity_list[element][QUANTITY_INDEX]
        periodic_table_value = periodic_table_dict[symbol]
        atomic_mass = periodic_table_value[ATOMIC_MASS_INDEX]
        total_atomic_mass = quantity * atomic_mass
        molar_mass += total_atomic_mass
...
def get_formula_name(formula, known_molecules_dict):
    ...
    if (formula in known_molecules_dict):
        compoundName = known_molecules_dict[formula]
    else:
